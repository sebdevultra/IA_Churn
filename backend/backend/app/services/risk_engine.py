...
class RiskEngine:
    ...
    def calculate_risk(
        ...
        # 8. Enterprise Sarcasm Rule (Piso 80 pts / Crítico)
        is_enterprise = bool(customer.tier and customer.tier.lower() == "enterprise")
        if is_enterprise and getattr(ai_output, "is_sarcastic", False):
            if raw_score < RiskWeightsConfig.ENTERPRISE_SARCASM_MIN_SCORE:
                sarcasm_diff = RiskWeightsConfig.ENTERPRISE_SARCASM_MIN_SCORE - raw_score
                factors.append(ScoreFactor(
                    rule_name="ENTERPRISE_SARCASM_THRESHOLD",
                    weight=sarcasm_diff,
                    applied=True,
                    reason="Alerta Crítica: Sarcasmo e ironía detectados en cuenta Enterprise (Piso 80 pts)"
                ))
                summary_reasons.append("Sarcasmo / Ironía en cuenta Enterprise (Crítico)")
                raw_score = RiskWeightsConfig.ENTERPRISE_SARCASM_MIN_SCORE

        # 9. Confidence Dampening
