...
class DeduplicationService:
    ...
    @classmethod
    def generate_hash(cls, customer_external_id: str, content: str, external_ref: str = None) -> str:
        """
        ...
        if external_ref:
            payload = f"{clean_cust_id}::{external_ref.strip()}::{clean_content}".encode("utf-8")
        else:
