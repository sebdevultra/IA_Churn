...
def get_kpis_for_horizon_ui(db: Session = Depends(get_db)):
    ...
    for it in interactions:
        ...
        alert = db.query(Alert).filter(Alert.churn_risk_id == r.id).first()
        is_resolved = (alert and alert.status in ["RESOLVED", "CLOSED"])
    ...
    return {
        ...
        "data": {
            ...
            "highRiskCount": active_high_count,
            "criticalRiskCount": active_critical_count
        }
    }
