...
class UIController {
  ...
  renderHighRiskTable(cases) {
    ...
    const filteredCases = cases.filter(item => {
      const normStatus = (item.status === 'PENDING' || item.status === 'NEW') ? 'PENDING'
        : (item.status === 'IN_REVIEW' || item.status === 'ACKNOWLEDGED') ? 'IN_REVIEW' : 'RESOLVED';

      // Status Filter
      if (this.activeFilter === 'CRITICAL' && item.riskScore < 80) return false;
      if (this.activeFilter === 'PENDING' && normStatus === 'RESOLVED') return false;
  ...
  _generateRowHTML(item) {
    ...
    const statusClass = normStatus === 'PENDING' ? 'status-pending'
      : normStatus === 'IN_REVIEW' ? 'status-review' : 'status-resolved';

    const statusLabel = normStatus === 'PENDING' ? 'Pendiente'
      : normStatus === 'IN_REVIEW' ? 'En Revisión' : 'Resuelto';
  ...
  openInterventionModal(caseItem) {
    ...
    if (factorsContainer && Array.isArray(caseItem.scoreFactors)) {
      factorsContainer.innerHTML = caseItem.scoreFactors.map(f => {
        const factorName = f.factor || f.reason || f.rule_name || 'Factor de Riesgo';
        const impactVal = f.impact !== undefined ? f.impact : (f.weight !== undefined ? f.weight : 0);
        const sign = impactVal > 0 ? '+' : '';
        return `
        <div class="factor-item">
          <span class="factor-name"><i class="fa-solid fa-angle-right"></i> ${this._escapeHTML(factorName)}</span>
          <span class="factor-impact">${sign}${impactVal} pts</span>
        </div>
      `;
      }).join('');
    }
    ...
    // Status Radio Inputs
    const normModalStatus = (caseItem.status === 'NEW' || caseItem.status === 'PENDING') ? 'PENDING'
      : (caseItem.status === 'IN_REVIEW' || caseItem.status === 'ACKNOWLEDGED') ? 'IN_REVIEW'
      : 'RESOLVED';
    const statusRadio = document.querySelector(`input[name="modal-status"][value="${normModalStatus}"]`);
